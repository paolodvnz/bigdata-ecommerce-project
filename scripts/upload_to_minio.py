...
Uploads generated datasets to S3-compatible MinIO storage
"""
import sys
...
def upload_dataset(client, source_dir: Path, prefix: str, dataset_name: str):
    """
    Upload dataset to MinIO (unified for RAW and SAMPLE)
    
    Args:
        client: MinIO client
        source_dir: Source directory (RAW_DIR or SAMPLE_DIR)
        prefix: Prefix for MinIO objects (e.g., "raw/" or "sample/")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Upload BigData E-commerce dataset to MinIO'
    )
    parser.add_argument(
        '--mode',
        type=str,
        ...
        default='sample',
        help='Dataset to upload: sample, full, or both'
    )
...
def main():
    ...
    if args.mode != 'sample':
        ...
        print("  1. RAW dataset only (FULL 100M transactions)")
        print("  2. SAMPLE dataset only (100K transactions)")
